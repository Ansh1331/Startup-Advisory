...
export function AppointmentCard({
  ...
  const canMarkCompleted = () => {
    if (userRole !== "ADVISOR" || appointment.status !== "SCHEDULED") {
      return false;
    }
    const now = new Date();
  ...
  // Handle save notes (advisor only)
  const handleSaveNotes = async () => {
    if (notesLoading || userRole !== "ADVISOR") return;
  ...
  const otherParty =
    userRole === "ADVISOR" ? appointment.founder : appointment.advisor;

  const otherPartyLabel = userRole === "ADVISOR" ? "Founder" : "Advisor";
  const otherPartyIcon = userRole === "ADVISOR" ? <User /> : <GraduationCap />;

  return (
    <>
      <Card className="border-cyan-900/20 hover:border-cyan-700/30 transition-all">
              ...
              <Badge
                ...
                className={
                  appointment.status === "COMPLETED"
                    ? "bg-cyan-900/20 border-cyan-900/30 text-cyan-300"
        ...
        <DialogContent className="max-w-2xl">
          ...
          <div className="space-y-4 py-4">
            ...
            <div className="space-y-2">
              ...
              <div className="flex items-center">
                <div className="h-5 w-5 text-cyan-300 mr-2">
                  {otherPartyIcon}
                </div>
                <div>
            ...
            {/* Founder Description */}
            {appointment.founderDescription && (
              <div className="space-y-2">
                <h4 className="text-sm font-medium text-muted-foreground">
                  {userRole === "ADVISOR"
                    ? "Founder Description"
                ...
                <div className="p-3 rounded-md bg-muted/20 border border-cyan-900/20">
            ...
            {/* Advisor Notes (Advisor can view/edit, Founder can only view) */}
            <div className="space-y-2">
                ...
                {userRole === "ADVISOR" &&
                  ...
                  appointment.status !== "CANCELLED" && (
                    <Button
                      ...
                      className="h-7 text-cyan-300 hover:text-cyan-300 hover:bg-cyan-900/20"
                    >
              ...
              {userRole === "ADVISOR" && action === "notes" ? (
                <div className="space-y-3">
                  <Textarea
                    value={notes}
                    onChange={(e) => setNotes(e.target.value)}
                    placeholder="Enter your clinical notes here..."
                    className="bg-background border-cyan-900/20 min-h-[100px]"
                  />
                ...
                <div className="p-3 rounded-md bg-muted/20 border border-cyan-900/20 min-h-[80px]">
                  {appointment.notes ? (
                    <p className="text-white whitespace-pre-line">
                      {appointment.notes}
                    </p>
          ...
          <DialogFooter className="flex flex-col-reverse sm:flex-row sm:justify-between sm:space-x-2">
            <div className="flex gap-2">
              {/* Mark as Complete Button - Only for advisors */}
              {canMarkCompleted() && (
                <Button
                  onClick={handleMarkCompleted}
                  disabled={completeLoading}
