...
export function AppointmentForm({ advisorId, slot, onBack, onComplete }) {
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6">
      <div className="bg-muted/20 p-4 rounded-lg border border-cyan-900/20 space-y-3">
        ...
        <div className="flex items-center">
          <CreditCard className="h-5 w-5 text-cyan-300 mr-2" />
      ...
      <div className="space-y-2">
        <Label htmlFor="description">
          Share your current business concern(optional)
        </Label>
        <Textarea
          id="description"
          placeholder="Please provide any details about your business concern or what you'd like to discuss in the appointment..."
          value={description}
          onChange={(e) => setDescription(e.target.value)}
          className="bg-background border-cyan-900/20 h-32"
        />
        <p className="text-sm text-muted-foreground">
          This information will be shared with the advisor before your
          appointment.
        </p>
      </div>
