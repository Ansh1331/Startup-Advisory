        ...
        <TabsList className="w-full justify-start overflow-x-auto">
          {days.map((day) => (
            ...
            >
              ...
              {day.slots.length > 0 && (
                <div className="ml-2 bg-cyan-900/30 text-cyan-300 text-xs px-2 py-1 rounded">
                  {day.slots.length}
                </div>
              )}
            </TabsTrigger>
              ...
              <div className="space-y-3">
                ...
                <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-2">
                  {day.slots.map((slot) => (
                    <Card
                      key={slot.startTime}
                      className={`border-cyan-900/20 cursor-pointer transition-all ${
                        selectedSlot?.startTime === slot.startTime
                          ? "bg-cyan-900/30 border-cyan-600"
                          : "hover:border-cyan-700/40"
                      }`}
